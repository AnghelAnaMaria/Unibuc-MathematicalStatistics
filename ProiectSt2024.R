...
for (i in 1:3){
  ...
  for (k in 1:discr){ #generam de 41 de ori esantioane de dimensiune n[i]
    ...
    #rbinom(n[i], 20, 1/10) generează un vector de dimensiune n[i] de eșantioane 
    #aleatoare dintr-o distribuție binomială de dimensiune = 20 și prob = 1/10. 
    #(rbinom(n[i], 20, 1/10) - 2) / (2/9) normalizarea datelor.
    #sum((rbinom(n[i], 20, 1/10) - 2) / (2/9)).Rezultatul transformării este apoi 
    #suma tuturor acestor eșantioane transformate. Această sumă este ulterior 
    #adăugată la vectorul bin.
    #bin <- c(bin, sum((rbinom(n[i], 20, 1/10) - 2) / (2/9))): În cele din urmă, 
    #valoarea rezultată este adăugată la vectorul bin folosind funcția c(). Astfel, 
    #la fiecare iterație a buclei interioare, un nou punct este adăugat la vectorul
    #bin, reprezentând suma eșantioanelor transformate pentru distribuția binomială 
    #la acea iterație specifică.
  ...
  #ECDF (Empirical Cumulative Distribution Function) reprezintă o funcție care 
  #atribuie fiecărei observații dintr-un set de date proporția sau probabilitatea 
  #de a fi mai mică sau egală cu o anumită valoare.
  plot(interval,abs(ecdf(bin)(bin)-pnorm(bin)),type="l",lwd="1",col="orange")
  #abs(ecdf(bin)(bin) - pnorm(bin)). Această expresie calculează diferența 
  #absolută între ECDF și funcția de distribuție normală standard (pnorm()) 
  #pentru fiecare valoare din vectorul bin (care conține eșantioanele din 
  #distribuția binomială).
  #ecdf(bin)(bin). Aici ecdf(bin) este o funcție care calculează ECDF pentru 
  #vectorul bin. (bin) este apoi aplicat pentru a evalua ECDF în punctele din bin.
  #pnorm(bin). Aceasta calculează valorile pentru funcția de distribuție normală 
  #standard pentru vectorul bin.
